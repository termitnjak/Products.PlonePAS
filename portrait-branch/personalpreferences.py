...
class ImageDisplayWidget(SimpleInputWidget):
    """Image display widget (renders OFS.Image.Image object """

    def __call__(self):
        """ Do not return anything, widget itself should not be rendered, 
            because current image is rendered from account-panel-bare template
            using tal:replace="structure widget/_getCurrentValue" """
        return ''
...
class UserDataPanel(AccountPanelForm):
    ...
    def __init__(self, context, request):
        ...
        if self.userid:
            member = mt.getMemberById(self.userid)
        else:
            member = mt.getAuthenticatedMember()
        if IChangePortraitCapability.providedBy(member):
            if not member.canModifyPortrait():
                self.form_fields['portrait'].custom_widget = ImageDisplayWidget
